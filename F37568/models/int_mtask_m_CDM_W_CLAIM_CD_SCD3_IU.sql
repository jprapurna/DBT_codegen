...
WITH SQ_CDH_GW_BUR AS (
    SELECT 
        ...
        BUR,          -- BUR information
        'GWCDH' AS SOURCE_NAME -- Name of the source
...
, LKP_W_CLAIM_CD_BUR_SCD3 AS (
    SELECT 
        lkp.W_CLAIM_CD_BUR_SCD3.ROW_WID AS lkp_ROW_WID,
        lkp.W_CLAIM_CD_BUR_SCD3.INTEGRATION_ID AS lkp_INTEGRATION_ID,
        lkp.W_CLAIM_CD_BUR_SCD3.NEW_BUR AS lkp_NEW_BUR,
        exp_bur.SOURCE_NAME,
        exp_bur.BUR,
        exp_bur.ROW_ID,
        exp_bur.in_INTEGRATION_ID
    FROM EXP_BUR AS exp_bur
    LEFT JOIN {{ source('CDM', 'W_CLAIM_CD_BUR_SCD3') }} AS lkp
    ON lkp.INTEGRATION_ID = exp_bur.in_INTEGRATION_ID
)


-- Mapplet CTE for mplt_CDM_BATCH_ID
, mplt_cdm_batch_id_cte AS (
    SELECT * FROM { mplt_cdm_batch_id(SOURCE_NAME) }
)
...
, EXP_Flag AS (
    ...
    FROM EXP_BUR
    LEFT JOIN LKP_W_CLAIM_CD_BUR_SCD3
        ON EXP_BUR.NK_OFFC_ID = LKP_W_CLAIM_CD_BUR_SCD3.NK_OFFC_ID
    LEFT JOIN mplt_cdm_batch_id_cte
        ON EXP_BUR.SOURCE_NAME = mplt_cdm_batch_id_cte.SOURCE_NAME
)
