...
WITH wrk_birp_ta_niss_nu0c_aprm_lnd AS (
    SELECT
        "NISS_APRM_LND_SK" AS niss_aprm_landing_sk, -- Primary key for NISS APRM landing
        "REG_PER_YR" AS registration_period_year, -- Registration period year
        ...
        "NJ_NO_LWST_LMT_IND" AS new_jersey_no_lowest_limit_indicator, -- New Jersey no lowest limit indicator
        "NJ_NMD_DRVR_EXCL_IND" AS new_jersey_named_driver_exclusion_indicator, -- New Jersey named driver exclusion indicator
        "EXPS_VAL_ROLLED" AS expense_value_rolled, -- Expense value rolled
        "COMP_DED" AS comprehensive_deductible, -- Comprehensive deductible
        ...
        "NUM_OF_CARS_IN_HH" AS number_of_cars_in_household, -- Number of cars in household
        "RDRVR_DT_OF_BRTH" AS rated_driver_date_of_birth, -- Rated driver date of birth
        "TERM_STRT_DT" AS term_start_date, -- Term start date
        "SRC_SYS_CD" AS source_system_code, -- Source system code
        "PNI_AGE" AS policyholder_age, -- Policyholder age
        "MIS_LOB" AS mis_line_of_business, -- MIS line of business
        "PRINCIPAL_OPRT" AS principal_operator, -- Principal operator
        "SOURCE_IND_DERIVED" AS source_indicator_derived -- Source indicator derived
    FROM {{ source('GENAI_POWER_BI', 'WRK_BIRP_TA_NISS_NU0C_APRM_LND') }}
)
SELECT
    niss_aprm_landing_sk,
    registration_period_year,
    fiscal_period_year,
    naic_company_code,
    niss_company_code,
    state_name,
    ...
    workflow_run_id,
    new_jersey_no_lowest_limit_indicator,
    new_jersey_named_driver_exclusion_indicator,
    expense_value_rolled,
    ...
    rated_driver_date_of_birth,
    term_start_date,
    source_system_code,
    policyholder_age,
    mis_line_of_business,
    principal_operator,
    source_indicator_derived
FROM wrk_birp_ta_niss_nu0c_aprm_lnd