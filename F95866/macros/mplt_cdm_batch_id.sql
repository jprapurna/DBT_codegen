...
with
    ...
    /* 2) Lookup transformation to retrieve maximum batch ID and trimmed source name */
    lkp_cdm_batch_ctrlid as (
        select
            max(BATCH_ID) as LKP_BATCH_ID,
            ltrim(rtrim(SOURCE_NAME)) as LKP_SOURCE_NAME
        from {{ source('CDM', 'CDM_BATCH_CTRLID') }}
        where STATUS = {{ var('status_running') }}
        group by SOURCE_NAME
    ),

    /* 3) Expression transformation to check for null values in batch ID */
    exp_null_check as (
        select
        ...
        on input_data.SOURCE_NAME = lkp_cdm_batch_ctrlid.LKP_SOURCE_NAME
    )
