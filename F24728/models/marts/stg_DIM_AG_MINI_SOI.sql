...
"RDRVR_AGE_RNGE" AS rdrvr_age_rnge, -- Rated driver age range
"RATE_CLS_CD" AS rate_cls_cd, -- Rate class code
"OCCUPTN_CD" AS occupnt_cd, -- Occupation code
"OCCUPTN_DESC" AS occupnt_desc, -- Occupation description
"OCCUPTN_CD_PRGM" AS occupnt_cd_prgm, -- Occupation code program
...
"ERLY_SHOPPNG_DISC_IND" AS erly_shopng_disc_ind, -- Early shopping discount indicator
"EFT_IND" AS eft_ind, -- EFT indicator
"PD_IN_FULL_IND" AS pd_in_full_ind, -- Paid in full indicator
"NON_FIG_HO_CRED_IND" AS non_fig_ho_cred_ind, -- Non-figured homeowner credit indicator
...
FROM {{ source('POWER_CENTER', 'DIM_AG_MINI_SOI') }}