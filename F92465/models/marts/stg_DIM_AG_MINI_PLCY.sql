...
WITH mini_policy AS (
    SELECT
        "MINI_PLCY_SK" AS mini_plcy_sk, -- Primary key for mini policy
        ...
        "PNI_AGE_RNGE" AS pni_age_rnge, -- Policyholder age range
        ...
        "PD_CRED_TIER_MDL_D" AS pd_cred_tier_mdl_d, -- Paid credit tier model D
        "UW_TIER_CD" AS uw_tier_cd, -- Underwriting tier code
        "DAYS_LPS_CD" AS days_lps_cd, -- Days lapse code
        "PRI_INS_NAIC_CMPY_CD" AS pri_ins_naic_cmpy_cd, -- Primary insurance NAIC company code
        "PRI_CARR_TYP" AS pri_carr_typ, -- Primary carrier type
        "PRI_BI_LMT_AMT" AS pri_bi_lmt_amt, -- Primary BI limit amount
        "PRI_BI_LMT_CD" AS pri_bi_lmt_cd, -- Primary BI limit code
        ...
        "NON_FIG_HO_CRED_IND" AS non_fig_ho_cred_ind, -- Non-figure homeowner credit indicator
        "TRNSFR_DISC_IND" AS trnsfr_disc_ind, -- Transfer discount indicator
        "FA2_PLCY_IND" AS fa2_plcy_ind, -- FA2 policy indicator
        "YTHFL_PNI_IND" AS ythfl_pni_ind, -- Youthful policyholder indicator
        ...
        "SUM_PLCY_CHK_SUM" AS sum_plcy_chk_sum, -- Summary policy checksum
        "SUM_PLCY_SK" AS sum_plcy_sk, -- Summary policy SK
        ...
        "CA_SNR_DRVR_DISC_IND" AS ca_snr_drvr_disc_ind, -- California senior driver discount indicator
        "CA_STDNT_AWAY_IND" AS ca_stdnt_away_ind -- California student away indicator
    FROM {{ source('GENAI_POWER_BI', 'DIM_AG_MINI_PLCY') }}
)
SELECT
    mini_plcy_sk,
    chk_sum_attr,
    ...
    ratng_cmpy_cd,
    ratng_cmpy_desc,
    first_prty_cred_tier_mdl_c,
    liab_cred_tier_mdl_a,
    pd_cred_tier_mdl_d,
    uw_tier_cd,
    days_lps_cd,
    pri_ins_naic_cmpy_cd,
    pri_carr_typ,
    pri_bi_lmt_amt,
    pri_bi_lmt_cd,
    bi_lmt,
    compre_wtht_coll_surchrg_ind,
    erly_shoppng_disc_ind,
    eft_ind,
    pd_in_full_ind,
    mlt_car_ind,
    mlt_line_ind,
