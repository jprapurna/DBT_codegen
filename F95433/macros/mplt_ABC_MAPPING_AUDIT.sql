...
expression_transformation AS (
  SELECT
    ...
    FOLDER_NAME,
    WORKFLOW_NAME,
    -- Local variable for record number increment
    ROW_NUMBER() OVER () AS v_RECORD_NUM,
    -- Mapping ID lookup
    CASE 
      WHEN ROW_NUMBER() OVER () = 1 THEN (
        CASE 
          WHEN :LKP.lkp_MAP_ID(MAPPING_NAME, FOLDER_NAME) IS NULL THEN NULL
          ELSE :LKP.lkp_MAP_ID(MAPPING_NAME, FOLDER_NAME)
        END
      )
      ELSE NULL
    END AS v_MAPNG_ID,
    ...
    CASE 
      WHEN ROW_NUMBER() OVER () = 1 THEN (
        CASE 
          WHEN :LKP.LKP_WORKFLOW_RUN_ID_ABC(WORKFLOW_NAME) IS NULL THEN :LKP.lkp_WORKFLOW_RUN_ID(WORKFLOW_NAME)
