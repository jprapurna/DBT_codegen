...
WITH soi_enhancement AS (
    SELECT
        "SOI_ENH_SK" AS soi_enh_sk, -- Primary key for SOI enhancement
        "PLCY_ID_SK" AS plcy_id_sk, -- Policy ID SK
        ...
        "NON_SMKR" AS non_smkr, -- Non-smoker
    ...
    FROM {{ source('GENAI_POWER_BI', 'DIM_AG_SOI_ENH') }}
)
SELECT
    soi_enh_sk,
    plcy_id_sk,
    ...
    add_unit_ind,
    rplc_veh_ind,
    distnt_stdnt_disc_ind,
    auto_splty_mtrc_disc_ind,
    auto_splty_boat_wc_disc_ind,
    auto_splty_offrd_oth_disc_ind,
    auto_splty_mtrhm_trlr_disc_ind,
    drvr_imprv_disc_ind,
    defns_drvr_disc_ind,
    cr_by_mapng_id,
    dw_cr_tmsp,
